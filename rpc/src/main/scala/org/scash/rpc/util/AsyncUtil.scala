...
abstract class AsyncUtil extends BitcoinSLogger {
  ...
  protected def retryUntilSatisfiedWithCounter(
    ...
    conditionF().flatMap { condition =>
      if (condition) {
        ...
        Future.failed(
          RpcRetryException(s"Condition timed out after $maxTries attempts with $duration waiting periods", stackTrace)
        )
      } else {
        ...
        p.future.flatMap {
          ...
          case false =>
            retryUntilSatisfiedWithCounter(conditionF, duration, counter + 1, maxTries, stackTrace)
        }
      }
    }
  }
  ...
  def awaitConditionF(
    ...
    retryUntilSatisfiedF(conditionF = conditionF, duration = duration, maxTries = maxTries)
...
object AsyncUtil extends AsyncUtil {
   ...
   */
  private[scash] val DEFAULT_INTERNVAL: FiniteDuration = 100.milliseconds

  /**
   * The default number of async attempts before timing out
   */
  private[scash] val DEFAULT_MAX_TRIES: Int = 50
