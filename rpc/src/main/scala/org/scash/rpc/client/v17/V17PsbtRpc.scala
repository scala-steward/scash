...
import org.scash.rpc.client.common.RpcOpts.WalletCreateFundedPsbtOptions
...
trait V17PsbtRpc { self: Client =>
  ...
  def createPsbt(
    ...
    bitcoindCall[String](
      "createpsbt",
      List(Json.toJson(inputs), outputsJson, JsNumber(locktime), JsBoolean(replacable))
    )
  }
  ...
  def finalizePsbt(psbt: String, extract: Boolean = true): Future[FinalizePsbtResult] =
    bitcoindCall[FinalizePsbtResult]("finalizepsbt", List(JsString(psbt), JsBoolean(extract)))
  ...
  def walletProcessPsbt(
    psbt: String,
    sign: Boolean = true,
    sighashType: SigHashType = SigHashType.bchALL,
    bip32Derivs: Boolean = false
  ): Future[WalletProcessPsbtResult] = {
    val args = List(JsString(psbt), JsBoolean(sign), Json.toJson(sighashType), JsBoolean(bip32Derivs))
