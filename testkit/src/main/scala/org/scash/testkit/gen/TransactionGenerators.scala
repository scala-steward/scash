...
trait TransactionGenerators extends BitcoinSLogger {
   ...
   */
  def signedP2PKTransaction: Gen[(TxSigComponent, ECPrivateKey)] =
    for {
   ...
   */
  def signedP2PKHTransaction: Gen[(TxSigComponent, ECPrivateKey)] =
    for {
   ...
   */
  def signedMultiSigTransaction: Gen[(TxSigComponent, Seq[ECPrivateKey])] =
    for {
   ...
   */
  def signedP2SHTransaction: Gen[(TxSigComponent, Seq[ECPrivateKey])] =
    for {
   ...
   * @return
   */
  def unspendableCLTVTransaction: Gen[(TxSigComponent, Seq[ECPrivateKey])] =
    for {
      (cltvLockTime, txLockTime) <- unspendableCLTVValues
      sequence                   <- NumberGenerator.uInt32s.suchThat(n => n < UInt32.max)
      (scriptSig, scriptPubKey, privKeys) <- ScriptGenerators.signedCLTVScriptSignature(
                                              cltvLockTime,
                                              txLockTime,
                                              sequence
                                            )
   ...
   */
  def spendableCLTVTransaction: Gen[(TxSigComponent, Seq[ECPrivateKey])] =
    for {
   ...
   */
  def spendableCSVTransaction: Gen[(TxSigComponent, Seq[ECPrivateKey])] =
    for {
  ...
  def unspendableCSVTransaction: Gen[(TxSigComponent, Seq[ECPrivateKey])] =
    for {
      (csvScriptNum, sequence) <- unspendableCSVValues
      tx                       <- csvTransaction(csvScriptNum, sequence)
    } yield tx

  def csvTransaction(csvScriptNum: ScriptNumber, sequence: UInt32): Gen[(TxSigComponent, Seq[ECPrivateKey])] =
    for {
      (signedScriptSig, csvScriptPubKey, privateKeys) <- ScriptGenerators.signedCSVScriptSignature(
                                                          csvScriptNum,
                                                          sequence
                                                        )
   ...
   */
  def spendableMultiSigEscrowTimeoutTransaction(outputs: Seq[TransactionOutput]): Gen[TxSigComponent] =
    for {
      sequence <- NumberGenerator.uInt32s
      amount   <- CurrencyUnitGenerator.satoshis
      (scriptSig, scriptPubKey, privKeys) <- ScriptGenerators.signedMultiSigEscrowTimeoutScriptSig(
                                              sequence,
                                              outputs,
                                              amount
                                            )
   ...
   */
  def spendableTimeoutEscrowTimeoutTransaction(outputs: Seq[TransactionOutput]): Gen[TxSigComponent] =
    for {
      (csvScriptNum, sequence) <- spendableCSVValues
      (scriptSig, scriptPubKey, privKeys) <- ScriptGenerators.timeoutEscrowTimeoutScriptSig(
                                              csvScriptNum,
                                              sequence,
                                              outputs
                                            )
  ...
  def spendableEscrowTimeoutTransaction(outputs: Seq[TransactionOutput] = Nil): Gen[TxSigComponent] =
    Gen.oneOf(spendableMultiSigEscrowTimeoutTransaction(outputs), spendableTimeoutEscrowTimeoutTransaction(outputs))

  /** Generates a [[Transaction]] that has a valid [[EscrowTimeoutScriptSignature]] */
  def spendableEscrowTimeoutTransaction: Gen[TxSigComponent] =
    Gen.oneOf(spendableMultiSigEscrowTimeoutTransaction(Nil), spendableTimeoutEscrowTimeoutTransaction(Nil))

  /** Generates a CSVEscrowTimeoutTransaction that should evaluate to false when run through the [[ScriptInterpreter]] */
  def unspendableTimeoutEscrowTimeoutTransaction: Gen[TxSigComponent] =
    for {
      (csvScriptNum, sequence)            <- unspendableCSVValues
  ...
  def unspendableMultiSigEscrowTimeoutTransaction: Gen[TxSigComponent] =
    for {
      sequence                   <- NumberGenerator.uInt32s
  ...
  private def lockTimeTxHelper(
  ...
  ): (TxSigComponent, Seq[ECPrivateKey]) = {
    ...
    val txSig  = TxSigComponent(signedSpendingTx, inputIndex, output, Policy.standardScriptVerifyFlags)
    (txSig, privKeys)
  }
  ...
  private def cltvLockTimesOfSameType(num: ScriptNumber, lockTime: UInt32): Boolean = num.toLong match {
    ...
    case positive if positive >= 0 =>
      if (!((lockTime < TransactionConstants.locktimeThreshold &&
