...
sealed abstract class ScriptGenerators extends BitcoinSLogger {
  ...
  /**
   * Generates a [[org.scash.core.protocol.script.P2SHScriptSignature]]
   * WARNING: the redeem script and the script signature DO NOT evaluate to true
   * if executed by [[org.scash.core.script.interpreter.ScriptInterpreter]]
   */
  ...
  def cltvScriptPubKey(num: ScriptNumber): Gen[(CLTVScriptPubKey, Seq[ECPrivateKey], ScriptNumber)] =
    for {
  ...
  def csvScriptPubKey(num: ScriptNumber): Gen[(CSVScriptPubKey, Seq[ECPrivateKey], ScriptNumber)] =
    for {
      (scriptPubKey, privKeys) <- randomNonLockTimeNonP2SHScriptPubKey
    } yield {
  ...
  def escrowTimeoutScriptPubKey: Gen[(EscrowTimeoutScriptPubKey, Seq[ECPrivateKey])] =
    for {
  ...
  def escrowTimeoutScriptPubKey2Of2: Gen[(EscrowTimeoutScriptPubKey, Seq[ECPrivateKey])] =
    for {
  ...
  def signedMultiSignatureScriptSignature
    : Gen[(MultiSignatureScriptSignature, MultiSignatureScriptPubKey, Seq[ECPrivateKey])] =
    for {
      (privateKeys, requiredSigs) <- CryptoGenerators.privateKeySeqWithRequiredSigs
      hashType                    <- CryptoGenerators.bchHashType
  ...
  def signedMultiSigEscrowTimeoutScriptSig(
    ...
    amount: CurrencyUnit
  ): Gen[(EscrowTimeoutScriptSignature, EscrowTimeoutScriptPubKey, Seq[ECPrivateKey])] =
    for {
  ...
  private def lockTimeHelper(
    ...
    val (unsignedSpendingTx, inputIndex) = TransactionGenerators.buildSpendingTransaction(
      tc.validLockVersion,
      creditingTx,
      EmptyScriptSignature,
      outputIndex,
    ...
    val txSignatureComponent = TxSigComponent(unsignedSpendingTx, inputIndex, output, Policy.standardScriptVerifyFlags)
  ...
  private def csvEscrowTimeoutHelper(
    ...
    hashType: SigHashType,
    isMultiSig: Boolean,
    outputs: Seq[TransactionOutput] = Nil,
    amount: CurrencyUnit = CurrencyUnits.zero
  ): EscrowTimeoutScriptSignature = {
    val pubKeys = privateKeys.map(_.publicKey)
    val (creditingTx, outputIndex) =
      TransactionGenerators.buildCreditingTransaction(TransactionConstants.validLockVersion, csvEscrowTimeout, amount)
  ...
  private def packageToSequenceOfPrivateKeys(
    gen: Gen[(ScriptSignature, ScriptPubKey, ECPrivateKey)]
