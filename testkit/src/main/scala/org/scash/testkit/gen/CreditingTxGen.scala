...
sealed abstract class CreditingTxGen {
  ...
  def output: Gen[BitcoinUTXOSpendingInfo] = Gen.oneOf(p2pkOutput, p2pkhOutput, multiSigOutput, p2shOutput, csvOutput)
        ...
        case (spk, keys) =>
          CryptoGenerators.bchHashType.map { hashType =>
            ...
            BitcoinUTXOSpendingInfo(
              ...
              TransactionOutput(creditingTx.outputs(outputIndex).value, creditingTx.outputs(outputIndex).scriptPubKey),
              signers,
              Some(spk),
              hashType
            )
  ...
  private def build(spk: ScriptPubKey, signers: Seq[Sign]): Gen[BitcoinUTXOSpendingInfo] = nonEmptyOutputs.flatMap {
    outputs =>
