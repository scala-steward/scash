  ...
  def isRedeemScript(token: ScriptToken): Boolean = {
    ...
    redeemScriptTry match {
      case Success(redeemScript) =>
        redeemScript match {
          case _: P2PKHScriptPubKey | _: MultiSignatureScriptPubKey | _: P2SHScriptPubKey | _: P2PKScriptPubKey |
              _: CLTVScriptPubKey | _: CSVScriptPubKey | _: EscrowTimeoutScriptPubKey =>
            true
  ...
  def isMultiSignatureScriptSignature(asm: Seq[ScriptToken]): Boolean = asm.isEmpty match {
    ...
    case false =>
      ...
      val restOfScriptIsPushOpsOrScriptConstants = asm.tail
        .map(token => token.isInstanceOf[ScriptConstant] || StackPushOperationFactory.isPushOperation(token))
        .exists(_ == false)
...
object ScriptSignature extends ScriptFactory[ScriptSignature] {
   ...
   * @return
   */
  def fromScriptPubKey(tokens: Seq[ScriptToken], scriptPubKey: ScriptPubKey): Try[ScriptSignature] =
    scriptPubKey match {
...
object EscrowTimeoutScriptSignature extends Factory[EscrowTimeoutScriptSignature] {
  ...
  def fromAsm(asm: Seq[ScriptToken], scriptPubKey: EscrowTimeoutScriptPubKey): EscrowTimeoutScriptSignature = {
    require(
      isEscrowTimeoutScriptSig(asm, Some(scriptPubKey)),
  ...
  def isEscrowTimeoutScriptSig(
    asm: Seq[ScriptToken],
    scriptPubKey: Option[EscrowTimeoutScriptPubKey] = None
  ): Boolean = {
    val nested       = asm.take(asm.length - 1)
    val last         = asm.last
  ...
  def apply(scriptSig: ScriptSignature): Try[EscrowTimeoutScriptSignature] = scriptSig match {
    ...
    case x @ (_: P2PKScriptSignature | _: P2PKHScriptSignature | _: P2SHScriptSignature |
        _: NonStandardScriptSignature | _: EscrowTimeoutScriptSignature | EmptyScriptSignature) =>
      Failure(new IllegalArgumentException("Cannot create a EscrowTimeoutScriptSignature out of " + x))

  }

  /**
   * Creates a [[org.scash.core.protocol.script.EscrowTimeoutScriptSignature]] that spends the escrow
  ...
  /**
   * Creates a [[org.scash.core.protocol.script.EscrowTimeoutScriptSignature]] that spends the locktime branch
