...
import org.scash.core.protocol._
import org.scash.core.script.bitwise.{ OP_EQUAL, OP_EQUALVERIFY }
import org.scash.core.script.constant.{ BytesToPushOntoStack, _ }
import org.scash.core.script.control.{ OP_ELSE, OP_ENDIF, OP_IF }
import org.scash.core.script.crypto.{ OP_CHECKMULTISIG, OP_CHECKMULTISIGVERIFY, OP_CHECKSIG, OP_HASH160 }
import org.scash.core.script.locktime.{ OP_CHECKLOCKTIMEVERIFY, OP_CHECKSEQUENCEVERIFY }
import org.scash.core.script.reserved.UndefinedOP_NOP
import org.scash.core.script.stack.{ OP_DROP, OP_DUP }
...
import org.scash.core.crypto.{ ECPublicKey, Sha256Hash160Digest }
...
sealed trait ScriptPubKey extends NetworkElement {
   ...
   * for the [[org.scash.core.protocol.CompactSizeUInt]] in the [[org.scash.core.protocol.script.ScriptPubKey]]
   */
...
object P2PKHScriptPubKey extends ScriptFactory[P2PKHScriptPubKey] {
  ...
  def isP2PKHScriptPubKey(asm: Seq[ScriptToken]): Boolean = asm match {
    case List(OP_DUP, OP_HASH160, _: BytesToPushOntoStack, _: ScriptConstant, OP_EQUALVERIFY, OP_CHECKSIG) => true
    case _                                                                                                 => false
  }
}
...
sealed trait MultiSignatureScriptPubKey extends ScriptPubKey {
  ...
  def requiredSigs: Int = {
    ...
    val opCheckMultiSigIndex =
      if (asm.indexOf(OP_CHECKMULTISIG) != -1) asmWithoutPushOps.indexOf(OP_CHECKMULTISIG)
...
object MultiSignatureScriptPubKey extends ScriptFactory[MultiSignatureScriptPubKey] {
  ...
  private def isValidPubKeyNumber(token: ScriptToken): Boolean = token match {
    case constant: ScriptConstant =>
      constant.isInstanceOf[ScriptNumber] || ScriptNumber(constant.bytes) <= ScriptNumber(
...
object P2SHScriptPubKey extends ScriptFactory[P2SHScriptPubKey] {
  ...
  def isP2SHScriptPubKey(asm: Seq[ScriptToken]): Boolean = asm match {
    case OP_HASH160 :: (_: BytesToPushOntoStack) :: (_: ScriptConstant) :: OP_EQUAL :: Nil => true
    case _                                                                                 => false
  }
...
object P2PKScriptPubKey extends ScriptFactory[P2PKScriptPubKey] {
  ...
  def isP2PKScriptPubKey(asm: Seq[ScriptToken]): Boolean = asm match {
    case List(_: BytesToPushOntoStack, _: ScriptConstant, OP_CHECKSIG) => true
    case _                                                             => false
  }
...
sealed trait LockTimeScriptPubKey extends ScriptPubKey {
  ...
  def locktime: ScriptNumber =
    asm.head match {
      case sNum: ScriptNumberOperation => ScriptNumber(sNum.toLong)
...
object CLTVScriptPubKey extends ScriptFactory[CLTVScriptPubKey] {
  ...
  def isCLTVScriptPubKey(asm: Seq[ScriptToken]): Boolean =
    if (asm.head.isInstanceOf[BytesToPushOntoStack]) {
      ...
      asm.slice(0, 4) match {
        case (_: BytesToPushOntoStack) :: (_: ScriptConstant) :: OP_CHECKLOCKTIMEVERIFY :: OP_DROP :: Nil =>
          validScriptAfterLockTime(tailTokens)
        case _ => false
      }
    } else {
      val tailTokens = asm.slice(3, asm.length)
      if (P2SHScriptPubKey.isP2SHScriptPubKey(tailTokens) || tailTokens.contains(OP_CHECKLOCKTIMEVERIFY)) false
      else
        asm.slice(0, 3) match {
          case (_: ScriptNumberOperation) :: OP_CHECKLOCKTIMEVERIFY :: OP_DROP :: Nil =>
            validScriptAfterLockTime(tailTokens)
          case _ => false
        }
    }
...
object CSVScriptPubKey extends ScriptFactory[CSVScriptPubKey] {
  ...
  def isCSVScriptPubKey(asm: Seq[ScriptToken]): Boolean =
    if (asm.head.isInstanceOf[BytesToPushOntoStack]) {
      ...
      asm.slice(0, 4) match {
        case (_: BytesToPushOntoStack) :: (_: ScriptConstant) :: OP_CHECKSEQUENCEVERIFY :: OP_DROP :: Nil =>
      ...
      if (P2SHScriptPubKey.isP2SHScriptPubKey(tailTokens) || tailTokens.contains(OP_CHECKSEQUENCEVERIFY)) return false
      asm.slice(0, 3) match {
        case (_: ScriptNumberOperation) :: OP_CHECKSEQUENCEVERIFY :: OP_DROP :: Nil =>
          CLTVScriptPubKey.validScriptAfterLockTime(tailTokens)
        case _ => false
      }
    }
