package org.scash.core.script.stack
...
sealed abstract class StackInterpreter {
  ...
  def opFromAltStack(p: ScriptProgram): ScriptProgram = {
    require(p.script.headOption.contains(OP_FROMALTSTACK), "Top of script stack must be OP_FROMALTSTACK")
    if (p.altStack.nonEmpty) ScriptProgram(p, p.altStack.head :: p.stack, p.script.tail, p.altStack.tail)
    else {
      logger.error("Alt Stack must have at least one item on it for OP_FROMALTSTACK")
      ScriptProgram(p, ScriptErrorInvalidAltStackOperation)
    }
  }
