...
sealed abstract class ArithmeticInterpreter {
  ...
  def opBoolOr(program: ScriptProgram): ScriptProgram = {
    ...
    binaryBoolOp(program, (x, y) => if (x == y && (x == ScriptNumber.zero || x == OP_0)) false else true)
  }
      ...
      case Some(s: ScriptConstant) =>
        if (ScriptFlagUtil.requireMinimalData(program.flags) && !BitcoinScriptUtil.isMinimalEncoding(s)) {
          ...
          unaryScriptNumber(
            ScriptProgram(
              program,
              ScriptNumber(ScriptNumberUtil.toLong(s.hex)) :: program.stack.tail,
              ScriptProgram.Stack
            )
          )
   ...
   */
  @tailrec
  private def binaryScriptNumbers(program: ScriptProgram): ScriptProgram \/ (ScriptNumber, ScriptNumber) =
    (program.stack.head, program.stack.tail.head) match {
      case (x: ScriptNumber, y: ScriptNumber) =>
        if (ScriptFlagUtil.requireMinimalData(program.flags) && (!BitcoinScriptUtil.isMinimalEncoding(x) || !BitcoinScriptUtil
              .isMinimalEncoding(y))) {
      ...
      case (x: ScriptConstant, y: ScriptConstant) =>
        binaryScriptNumbers(
          ScriptProgram(
            program,
            ScriptNumber(x.hex) :: ScriptNumber(y.hex) :: program.stack.tail.tail,
            ScriptProgram.Stack
          )
        )
